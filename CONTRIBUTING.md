...
`docs/src/pages/llms.txt.ts` and `llms-full.txt.ts` generate `/llms.txt` (an index of every page with its description) and `/llms-full.txt` (the whole English corpus as plain Markdown, ~82 kB) from the docs collection, in sidebar order — shared helpers live in `docs/src/lib/llms.ts`. Neither is hand-edited, and a page missing from the `sidebar` array reaches neither file; `test/docs-llms-txt.test.mjs` guards that. `toMarkdown` rewrites MDX to plain Markdown and must stay fence-aware: example code contains its own `import` lines and `#` comments, and rewriting those would corrupt the snippets.
