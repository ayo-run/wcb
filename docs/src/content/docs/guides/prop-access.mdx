...
(see the caution above) and override the converters, exactly as `EventCard`
does for its `Date`. Here is a `Set` reflecting as a space-delimited token
list, the same shape native `rel` and `class` use:

```js
class TagList extends WebComponent {
  static props = { tags: new Set() }

  toAttribute(name, value) {
    if (name === 'tags') return value.size ? [...value].join(' ') : null
    return super.toAttribute(name, value)
  }

  fromAttribute(name, value) {
    if (name === 'tags') return new Set(value.split(/\s+/).filter(Boolean))
    return super.fromAttribute(name, value)
  }
}
```
...
```html
<tag-list tags="alpha bravo"></tag-list>
<!-- props.tags is a real Set -->
```
...
Converters cover any value with a sensible **textual form** — a `Date` as an
ISO date, a `Set` as a token list, a `URL` as its `href`. A value with no
textual form at all — a function, an element reference, a live handle like an
`AbortController` — is not a converter problem: it doesn't belong in
`static props` in the first place. Keep it as a plain class property instead —
see [Unobserved properties](#unobserved-properties) below. wcb warns once per
