...
title: Prop Access
slug: prop-access
description: Read and write observed attributes as camelCase through this.props, with typed props, boolean semantics and custom attribute conversion.
---
...
<Aside type="tip" title="Use `static strictProps` if you want runtime type guards">
...
For `null` and `undefined`, the default conversion removes the attribute.
That is, `this.props.selected = undefined` removes
`selected` rather than writing `selected="undefined"`. A nullish default in
`static props` reflects as no attribute at all. Under the hood, the prop
stays nullish as WCB recognises its own reflection.
