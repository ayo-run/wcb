...
description: Scaffold a component with npm create wcb, or install web-component-base and write your first reactive custom element by hand.
---
...
- **A starter component** in TypeScript. The class, tag, and file names are generated from your project name (`my-button` → `MyButton` / `<my-button>`)
- **A dev server setup**: a Vite project with a demo page, plus a library build (`npm run build:lib`) that emits ESM + UMD bundles and `.d.ts` types
- **Automated manifest generation for tooling integrations**:
    - the CEM analyzer configured with wcb's plugins
    - an `npm run analyze` script,
    - the `customElements` field in `package.json`,
    - and a `prepack` hook so the manifest ships inside the published package.
...
<Aside type="tip" title="You will find generated files collected inside a `.wcb` directory">
    All generated output files, including the custom elements manifest and code editor autocomplete integrations (VS Code custom-data files and a JetBrains web-types file), lands in a gitignored `.wcb` directory.
</Aside>
...
Open a [GitHub issue](https://github.com/ayo-run/wcb/issues/new) or [discussion](https://github.com/ayo-run/wcb/discussions) for problems or requests. You can also submit a ticket on [SourceHut](https://todo.sr.ht/~ayoayco/wcb) or via [Email](mailto:~ayoayco/wcb@todo.sr.ht).
