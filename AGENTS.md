...
- `pnpm release` cuts a root `web-component-base` release (tag `vX.Y.Z`); `pnpm release:create` cuts a `create-wcb` release (tag `create-wcb@X.Y.Z`). The two packages version independently and each tag namespace triggers its own workflow (`release.yml` / `release-create.yml`), which publishes to npm via trusted publishing — provenance is generated automatically, and **only** from GitHub Actions, never from a local `npm publish`. Both changelog jobs pin `--from` to the previous tag *in their own namespace*, because the namespaces interleave.
- Both release commands are self-contained — they gate on their own checks before anything is tagged or pushed, so a release is one command. `release` runs `release:check` (`test:all` + `size-limit` + a peer-range warning); `release:create` runs `release:create:check` (peer-range check, hard-failing, + `test:template`). The gate runs **before** `bumpp`, because once the tag is pushed the publish and changelog jobs are already in flight. The root `prepack` (`pnpm run build`) is the backstop underneath: `dist/` is gitignored and `files` is `["dist"]`, so without it a publish outside CI would ship an empty tarball.
- `create/template/package.json` pins `peerDependencies.web-component-base` to a caret range. A wcb minor/patch release is picked up by that range automatically, but a **major** wcb release strands new scaffolds on the old major until `create-wcb` is released with a bumped range. `pnpm test:template` symlinks the workspace root in, so it never exercises the published range — the drift is invisible to every test. `scripts/check-create-release.js` is what catches it: `--warn` during a wcb release (never block shipping wcb), hard-fail during a create-wcb release (there, the stale range *is* the bug).
